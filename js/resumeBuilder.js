var bio = {
	...
	"pic" : "images/dio.png",
	"welcomeMsg" : "I make front-end interfaces. They're responsive like cats to cucumbers and beautiful like Tom Hardy in every Tom Hardy film.",
	...
	display: function() {
		...
		var formattedBlog = HTMLblog.replace("%blog%", bio.contactInfo.blog).replace("%blogname%", bio.contactInfo.blogname);
...
var education = {
	...
	display: function() {
		for (k in education.schools) {
			...
			var formattedSchoolDates = HTMLschoolDates.replace("%data%", education.schools[k].gradyear);
		...
		for (g in education.onlineCourses) {
			$("#education").append(HTMLschoolStart);
			...
			var formattedOnlineDates = HTMLonlineDates.replace("%data%", education.onlineCourses[g].dates);
